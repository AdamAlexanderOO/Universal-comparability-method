...
export default function App() {
  ...
  return (
    <div
      ...
      className="relative min-h-screen w-full bg-[#0A0A0A] text-white font-sans flex flex-col selection:bg-red-600 selection:text-white pb-24 overflow-x-hidden"
    >
      ...
      {/* TOP SMARTPHONE STATUS & CONTROL BAR */}
      <header className="sticky top-0 z-40 border-b border-white/15 bg-[#0e0e0e]/95 backdrop-blur-md px-3.5 py-2.5 flex items-center justify-between select-none">
        {/* Left Branding */}
        <div className="flex items-center gap-2">
          <div className="w-2.5 h-2.5 bg-red-600 rounded-full animate-pulse shadow-[0_0_8px_#ef4444]" />
          <div>
            <div className="flex items-center gap-1.5 leading-none">
              <span className="font-black text-sm sm:text-base tracking-tighter text-white">IMAGER</span>
            ...
            <div className="text-[9px] font-mono text-neutral-400 mt-0.5">
              {fluxFrequency.toFixed(1)} GHz | {currentLight.name}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1.5 rounded bg-white/5 border border-white/15 hover:bg-white/10 active:scale-95 transition-all text-xs font-mono"
            title="Cycle Spectrum"
          >
            <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: currentLight.primaryColor }} />
            <span className="text-[10px] font-bold">{currentLight.wavelengthTHz}T</span>
          </button>
          ...
          <button
            ...
            className="p-2 rounded bg-white/5 border border-white/15 text-neutral-300 hover:text-white active:scale-95"
            title="Sound"
          >
      ...
      {/* QUICK HORIZONTAL SCROLLABLE TAB PILLS (One-touch switching on phone) */}
      <div className="border-b border-white/10 bg-[#080808] px-2.5 py-2 overflow-x-auto flex items-center gap-1.5 select-none scrollbar-none">
        {MOBILE_TABS.map((tab) => {
      ...
      {/* MOBILE EXECUTIVE TELEMETRY TICKER */}
      <div className="grid grid-cols-4 gap-1 p-2 bg-[#060606] border-b border-white/10 font-mono text-center select-none">
        ...
        <div className="p-1 border border-white/5 bg-white/[0.02] rounded">
          ...
          <div className="text-xs font-bold text-red-500">{subsystems.aiCore.load}%</div>
        </div>
        <div className="p-1 border border-white/5 bg-white/[0.02] rounded">
      ...
      <main className="flex-1 p-2.5 sm:p-4 max-w-3xl mx-auto w-full">
        ...
        {activeTab === 'CONSOLE' && (
          <div className="space-y-3">
            <div className="flex items-center justify-between pb-1.5 border-b border-white/10 font-mono text-xs">
              <span className="text-red-500 font-bold">01 // HARDWARE CHASSIS</span>
        ...
        {activeTab === 'PCB' && (
          <div className="space-y-3">
            <div className="flex items-center justify-between pb-1.5 border-b border-white/10 font-mono text-xs">
              <span className="text-red-500 font-bold">02 // LIGHT-PROTOCOL PCB</span>
              <button
                onClick={handleOverchargeCore}
                className="bg-red-600 text-white px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded active:scale-95 transition-all shadow-[0_0_8px_rgba(239,68,68,0.5)]"
              >
                Overcharge
              </button>
            </div>
        ...
        {activeTab === 'GEARS' && (
          <div className="space-y-3">
            <div className="flex items-center justify-between pb-1.5 border-b border-white/10 font-mono text-xs">
              <span className="text-red-500 font-bold">03 // HOLOGRAPHIC GEAR ENGINE</span>
        ...
        {activeTab === 'RADAR' && (
          <div className="space-y-3">
            <div className="flex items-center justify-between pb-1.5 border-b border-white/10 font-mono text-xs">
              <span className="text-red-500 font-bold animate-pulse">04 // TACTICAL RADAR HUD</span>
        ...
        {activeTab === 'SUBSYSTEMS' && (
          <div className="space-y-3">
            <div className="flex items-center justify-between pb-1.5 border-b border-white/10 font-mono text-xs">
              <span className="text-red-500 font-bold">05 // 8 SUBSYSTEMS MATRIX</span>
              <button
                onClick={handleOverchargeCore}
        ...
        {activeTab === 'ARCADE' && (
          <div className="space-y-5">
            <div className="flex items-center justify-between pb-1.5 border-b border-white/10 font-mono text-xs">
              <span className="text-red-500 font-bold">06 // CYBER ARCADE & REPL</span>
              <span className="text-neutral-400 font-mono">TOUCH READY</span>
            </div>
            <MiniGamesSuite powerOn={powerOn} fluxFrequency={fluxFrequency} />
            <TerminalArchiveView
              logs={terminalLogs}
        ...
        {/* CONTINUOUS FEED (ALL 6 MODULES STACKED VERTICALLY) */}
        {activeTab === 'ALL_FEED' && (
          <div className="space-y-8">
            <section className="space-y-2">
              <h2 className="text-xs font-mono font-bold text-red-500 pb-1 border-b border-white/10">01 // HARDWARE CHASSIS</h2>
              <div className="flex justify-center">
                <AuroraMachineConsole
                  powerOn={powerOn}
                  onTogglePower={() => setPowerOn(!powerOn)}
            ...
            <section className="space-y-2">
              <h2 className="text-xs font-mono font-bold text-red-500 pb-1 border-b border-white/10">02 // LIGHT-PROTOCOL PCB MATRIX</h2>
              <div className="border border-white/10 bg-neutral-950/80 p-2 sm:p-4 rounded-lg overflow-x-auto">
                <LightProtocolPCB
                  powerOn={powerOn}
                  subsystems={subsystems}
            ...
            <section className="space-y-2">
              <h2 className="text-xs font-mono font-bold text-red-500 pb-1 border-b border-white/10">03 // HOLOGRAPHIC GEAR ENGINE</h2>
              <div className="border border-white/10 bg-neutral-950/80 p-2 sm:p-4 rounded-lg">
                <HologramGearEngine
                  powerOn={powerOn}
                  fluxFrequency={fluxFrequency}
            ...
            <section className="space-y-2">
              <h2 className="text-xs font-mono font-bold text-red-500 pb-1 border-b border-white/10">04 // TACTICAL RADAR HUD</h2>
              <div className="border border-white/10 bg-neutral-950/80 p-2 sm:p-4 rounded-lg">
                <TacticalTelemetryHUD
                  powerOn={powerOn}
                  telemetry={telemetry}
            ...
            <section className="space-y-2">
              <h2 className="text-xs font-mono font-bold text-red-500 pb-1 border-b border-white/10">05 // 8 SUBSYSTEMS MATRIX</h2>
              <SubsystemsArchiveView
                subsystems={subsystems}
                currentLight={currentLight}
                powerOn={powerOn}
            ...
            <section className="space-y-2">
              <h2 className="text-xs font-mono font-bold text-red-500 pb-1 border-b border-white/10">06 // CYBER ARCADE & REPL</h2>
              <MiniGamesSuite powerOn={powerOn} fluxFrequency={fluxFrequency} />
              <TerminalArchiveView
                logs={terminalLogs}
                onClearLogs={() => setTerminalLogs([])}
      ...
      <nav aria-label="Smartphone Navigation" className="fixed bottom-0 left-0 right-0 z-40 bg-[#0c0c0c]/98 backdrop-blur-xl border-t border-white/15 px-2 py-1.5 flex items-center justify-around select-none">
        {MOBILE_TABS.map((tab) => {
          const IconComp = tab.icon;
          const isActive = activeTab === tab.id;
          return (
            ...
            >
              <IconComp className={`w-5 h-5 mb-0.5 ${isActive ? 'text-red-500 drop-shadow-[0_0_6px_rgba(239,68,68,0.8)]' : ''}`} />
